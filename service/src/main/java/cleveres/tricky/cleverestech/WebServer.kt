...
class WebServer(
    ...
    private fun readNativeRuntimeStatus(): JSONObject {
        ...
        // The latching TEE circuit breaker gates every future activation attempt,
        // so while it is set it is the live blocker: a stale injector snapshot
        // must not be presented as the current runtime state.
        if (KeystoreInterceptor.isTeeBroken()) {
            return JSONObject()
                .put("state", "failed")
                .put("alive", false)
                .put("failure", "tee_broken")
                .put("pid", 0)
                .put("entry", "unknown")
                .put("timestamp_ms", 0L)
        }
        ...
        return runCatching {
            ...
            // "alive" means the recorded runtime is the live, operating backend.
            // A failed or starting attempt must never report alive=true merely
            // because the recorded pid still exists.
            val alive =
                state == "active" &&
                    pid > 0 &&
                    recordedStartTicks > 0 &&
                    currentStartTicks == recordedStartTicks
            val knownFailures =
                setOf(
                    "none",
                    "request_validation",
    ...
    companion object {
        ...
        fun restoreBackupZip(
            ...
            try {
                ...
                sanitizeRestoredDisabledKeyboxes(staged)
        ...
        /**
         * Drops restored opt-out identifiers that name no staged source: a
         * syntactically valid entry can otherwise persist for a keybox that is
         * absent from the archive, and a later upload of the same scope and
         * filename would silently inherit the disabled state. Entries that
         * match a staged root or keyboxes source are kept unchanged.
         */
        private fun sanitizeRestoredDisabledKeyboxes(staged: MutableMap<String, ByteArray>) {
            val raw = staged[DISABLED_KEYBOXES_FILENAME] ?: return
            val stagedIds = HashSet<String>()
            if (staged.containsKey("keybox.xml")) {
                stagedIds += "${StoredKeyboxInventory.Scope.ROOT.apiValue}:keybox.xml"
            }
            staged.keys.asSequence()
                .filter { it.startsWith("keyboxes/") }
                .forEach { stagedIds += "${StoredKeyboxInventory.Scope.KEYBOXES.apiValue}:${it.substringAfter('/')}" }
            if (stagedIds.isEmpty()) {
                staged.remove(DISABLED_KEYBOXES_FILENAME)?.fill(0)
                return
            }
            val kept = StringBuilder()
            var dropped = false
            raw.toString(Charsets.UTF_8).lineSequence().forEach { rawLine ->
                val line = rawLine.trim()
                // Blank lines are tolerated to match the runtime reader.
                if (line.isEmpty()) return@forEach
                if (line in stagedIds) {
                    kept.append(line).append('\n')
                } else {
                    dropped = true
                    Logger.w("Dropping disabled keybox identifier without a staged source: $line")
                }
            }
            if (!dropped) return
            val previous = staged.getValue(DISABLED_KEYBOXES_FILENAME)
            if (kept.isEmpty()) {
                staged.remove(DISABLED_KEYBOXES_FILENAME)
            } else {
                staged[DISABLED_KEYBOXES_FILENAME] = kept.toString().toByteArray(Charsets.UTF_8)
            }
            previous.fill(0)
        }
         ...
         * Bindings naming no staged keybox are dropped too: restore deletes
         * every keybox absent from the archive, so a dangling binding can never
         * refer to a surviving file, and keeping it would let a later same-name
         * upload inherit trusted status and skip revocation checks.
         */
         private fun sanitizeRestoredRkpProvenance(staged: MutableMap<String, ByteArray>) {
            ...
            for (entry in entries) {
                ...
                if (stagedKeys == null) {
                    dropped++
                    Logger.w("Dropping dangling RKP provenance binding from restore: $normalized")
                    continue
                }
