  ...
  if (found) {
    ...
    if (gBinderInterceptor != nullptr &&
      ...
      int32_t exception_code =
          reply_parcel.readInt32(); // readExceptionCode usually just reads the
                                    // first int
      if (exception_code != 0) {
      ...
      if (readString16_manual(reply_parcel, spoofed_value)) {
        ...
        // LOGD("Property service returned null or failed to read for %s",
        // name);
      }
    } else {
...
int new_ioctl(int fd, unsigned long request, ...) {
  ...
  if (result >= 0 && request == BINDER_WRITE_READ) {
    ...
    // old_ioctl above would likely have failed (returning < 0) for
    // BINDER_WRITE_READ, causing us to skip this block anyway.
    if (!is_binder_fd(fd)) {
      return result;
    }
