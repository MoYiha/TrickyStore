...
assert.ok(/const\s+stalePatchHost\s*=\s*document\.getElementById\('ct_identity_patch'\)/.test(policy), 'retired embedded Security Patch host in Identity must be removed');
...
assert.ok(/const\s+stale\s*=\s*document\.getElementById\('ct_identity_controls'\)/.test(policy), 'legacy Identity-page toggle panel must be removed');
...
assert.ok(/securityPatch:\s*source\.securityPatch\s*===\s*true/.test(policy), 'Security Patch persistence must preserve the explicit global switch with strict boolean normalization');
...
assert.ok(/FEATURE_KEYS\.some\(\(\[key\]\)\s*=>\s*Boolean\(policyState\.features\[key\]\)\)/.test(policy), 'Identity master state must follow only its own identity features');
assert.ok(/\['securityPatch',\s*'Security Patch'/.test(policy), 'per-profile Security Patch compatibility must remain intact');
assert.ok(policy.includes('data-open-tab="patch"'), 'UI control on Dashboard must navigate to the Security Patch tab');

const bindStart = policy.search(/function\s+bindIdentityControls\s*\(panel,\s*prefix\)/);
const bindEnd = policy.indexOf('function installIdentityControls()', bindStart);
const bindBlock = policy.slice(bindStart, bindEnd);
assert.ok(!/next\.features\.securityPatch\s*=\s*enabled/.test(bindBlock), 'Identity master must not mutate the independent Security Patch feature');
...
assert.ok(/const\s+patchOn\s*=\s*Boolean\(policyState\.features\s*&&\s*policyState\.features\.securityPatch\)/.test(patchRenderBlock), 'detail controls must follow only the Security Patch switch');
assert.ok(!/next\.features\.securityPatch\s*=\s*(?:true|!0)/.test(patchRenderBlock), 'editing patch modes must not silently re-enable a disabled Security Patch feature');
