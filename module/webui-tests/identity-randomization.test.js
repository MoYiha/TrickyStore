...
assert.ok(/bindIdentityControls\(panel,\s*`\$\{prefix\}_identity`\)/.test(policy), 'Dashboard Identity controls must be bound');
...
assert.ok(/const\s+stale\s*=\s*document\.getElementById\('ct_identity_controls'\)/.test(policy), 'legacy Identity Controls panel cleanup is missing');
...
assert(/visible_sim_count:\s*'inputVisibleSimCount'/.test(index), 'random payload must map visible SIM count');
...
assert(/visible_camera_count:\s*'inputVisibleCameraCount'/.test(index), 'random payload must map visible camera count');
...
const visibilityBlockMatch = policy.match(/headers\.forEach\(\s*header\s*=>\s*\{[\s\S]*?\}\);/);
