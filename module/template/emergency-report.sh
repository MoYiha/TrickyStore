...
' "Acil durum raporunu doğrudan çalıştırdınız." \
                          "Bu işlem çok acil durumlarda tanı arşivi oluşturur." \
                          "Modül ayarlarını açmak için WebUI'yi kullanın (KernelSU/APatch'te modül düğmesi, Magisk'te Action düğmesi)." \
                          "13 saniye sonra rapor oluşturulacak."
            ;;
...
' "您直接运行了紧急情况报告。" \
                          "此操作用于在非常紧急的情况下生成诊断归档。" \
                          "如需打开模块设置，请使用 WebUI（KernelSU/APatch 上为模块按钮，Magisk 上为 Action 按钮）。" \
                          "13 秒后将生成报告。"
            ;;
...
' "Ejecutó el informe de emergencia directamente." \
                          "Esto genera un archivo de diagnóstico en situaciones muy urgentes." \
                          "Para abrir los ajustes del módulo, use la WebUI (botón del módulo en KernelSU/APatch, botón Action en Magisk)." \
                          "El informe se generará en 13 segundos."
            ;;
...
' "Sie haben den Notfallbericht direkt ausgeführt." \
                          "Damit wird in sehr dringenden Fällen ein Diagnosebericht erstellt." \
                          "Zum Öffnen der Moduleinstellungen verwenden Sie die WebUI (Modulseite unter KernelSU/APatch, Action-Schaltfläche unter Magisk)." \
                          "Der Bericht wird in 13 Sekunden erstellt."
            ;;
...
' "Вы запустили аварийный отчет напрямую." \
                          "Он создает диагностический архив в очень срочных ситуациях." \
                          "Чтобы открыть настройки модуля, используйте WebUI (кнопка модуля в KernelSU/APatch, кнопка Action в Magisk)." \
                          "Отчет будет создан через 13 секунд."
            ;;
...
' "Anda menjalankan laporan darurat secara langsung." \
                          "Ini membuat arsip diagnostik dalam situasi yang sangat mendesak." \
                          "Untuk membuka pengaturan modul, gunakan WebUI (tombol modul di KernelSU/APatch, tombol Action di Magisk)." \
                          "Laporan akan dibuat dalam 13 detik."
            ;;
...
' "आपने आपातकालीन रिपोर्ट सीधे चलाई है।" \
                          "यह बहुत जरूरी स्थितियों में डायग्नोस्टिक संग्रह बनाता है।" \
                          "मॉड्यूल सेटिंग्स खोलने के लिए WebUI का उपयोग करें (KernelSU/APatch पर मॉड्यूल बटन, Magisk पर Action बटन)।" \
                          "13 सेकंड में रिपोर्ट बनाई जाएगी।"
            ;;
...
' "لقد شغّلت تقرير الطوارئ مباشرة." \
                          "يؤدي هذا إلى إنشاء أرشيف تشخيصي في الحالات الطارئة جداً." \
                          "لفتح إعدادات الوحدة، استخدم WebUI (زر الوحدة على KernelSU/APatch، وزر Action على Magisk)." \
                          "سيتم إنشاء التقرير خلال 13 ثانية."
            ;;
...
' "You ran the emergency report directly." \
                          "This generates a diagnostic archive for very urgent situations." \
                          "To open the module settings, use the WebUI (module button on KernelSU/APatch, Action button on Magisk)." \
                          "The report will be generated in 13 seconds."
            ;;
    esac
}
