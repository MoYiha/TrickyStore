...
## 2026-06-15 - [Internal State Exposure via Public Interface]
**Vulnerability:** The `PropertyHiderService` exposed internal configuration keys (like `CODENAME`, `FINGERPRINT`) via the binder interface because it blindly proxied requests to `Config.getBuildVar`, which handles both public system properties and internal template variables. This allowed detection of the spoofing mechanism by querying these non-standard properties.
**Learning:** When using a single configuration store for both internal state and public-facing values, ensure the public interface (Binder) filters out internal-only keys.
**Prevention:** Maintain a strict whitelist of allowed keys or a blacklist of internal keys at the API boundary to prevent leakage of internal state.
