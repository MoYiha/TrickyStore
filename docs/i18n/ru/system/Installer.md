...
Ставит полный KernelSU/APatch/Magisk module на Android 12-17 ARM64/x86 64. Все три среды обеспечивают полную поддержку WebUI; в Magisk WebUI открывается кнопкой Action модуля через отдельное хост-приложение WebUI (см. [Поддержка Magisk](Magisk.md)). Установка через recovery отклоняется.
